...
// Expire time in seconds (aldatu daiteke gure behar izanetara, adi: 7 * 24 * 60 * 60)
const TTL = 60 * 60;
...
export class CachingService {
  ...
  async getCachedRequest(url): Promise<any> {
    ...
    url = `${CACHE_KEY}${url}`; const storedValue = await this.storage.get(url);
    if (!storedValue) {
      return null;
    } else if (storedValue.validUntil < currentTime) {
      await this.storage.remove(url); return null;
    } else { return storedValue.data; }
  }
